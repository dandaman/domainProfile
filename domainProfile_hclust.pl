...
#===============================================================================
#
#         FILE:  domainProfile.pl
#
#        USAGE:  ./domainProfile.pl  
#
...
my $USAGE   = "$0 -h | -o outpath -n profile_name [-l] [-p] [-m hclust_method[ward|single|complete|average|mcquitty|median|centroid]] [-r ratio_considered_as_representative -q qvalue_cutoff_for_outliers -a qvalue_cutoff_for_absent]  -s seq_file [-f format(fasta)] [-i ignore_motif_regex] [-d static_cut_at_tree_height] JALVIEW_ANNOT_FILE\n"; 

my ($opts_h, $opts_l, $opts_o, $opts_n, $opts_p, $opts_f, $opts_s, $opts_r, $opts_q, $opts_a, @opts_i,$criterion,$opts_v, $opts_d);
...
$r  = Statistics::R->new();
$opts_r = defined $opts_r && $opts_r > 0 && $opts_r<= 1 ? $opts_r : 0.9;
$opts_q = defined $opts_q && $opts_q > 0 && $opts_q<= 1 ? $opts_q : 0.05;
$opts_a = defined $opts_a && $opts_a > 0 && $opts_a<= 1 ? $opts_a : 0.9;

if ($criterion) {
	die "unknown method $criterion\n$USAGE" unless $criterion =~ /^ward|single|complete|average|mcquitty|median|centroid$/;
}
else {
	$criterion="ward";
}
...
print "Read sequence lengths of ", scalar(keys %L)," sequences\n" if $opts_v;
...
print "Read jalview annotation with ", scalar(keys %dom_freq)," domains/motifs\n" if $opts_v;
...
print "Got average motif/domain lengths\n" if $opts_v;
...
print "Got motif/domain penalties\n" if $opts_v;
...
print "Got maximal motif/domain length\n" if $opts_v;
...
$ic =0;
@IDs=sort grep {exists $motifs{$_}} keys %L;

print "Performing ",scalar(@IDs)*(scalar(@IDs)-1)," alignments (",scalar(@IDs),"/$seq_count sequences)\n" if $opts_v;
my $alns;
foreach my $i (@IDs) {
...
print "Performed motif/domain alignments for $ic $ec sequences\n" if $opts_v;
##############EXPORT MATRIX
...
print "Wrote matrix to $PN\n" if $opts_v;
##############CLUSTERING
if ($criterion) {
	$r->run("suppressPackageStartupMessages(library(fastcluster))");
	$r->run("suppressPackageStartupMessages(library(WGCNA))");
	$r->run("suppressPackageStartupMessages(library(dynamicTreeCut))");
	$r->run("suppressPackageStartupMessages(library(moduleColor))");
	$r->send(qq{M<-as.matrix(read.table("$PN",sep="\t",as.is=TRUE,header=TRUE,row.names=1))});
	$r->send(qq{m<-as.dist(M)});
	$r->send(qq{h<-hclust(m,method="$criterion")});
	if ($opts_d) {
#        https://stat.ethz.ch/pipermail/r-help/2008-May/163409.html
		$r->send(q{h$height <- round(h$height,6)});
		$r->send(qq{k<-cutree(h,h=$opts_d)});
	}
	else {
		$r->send(qq{k<-cutreeDynamic(h, distM=M , minClusterSize = 50)});
	}
	$r->send(qq{col<-labels2colors(k)});

    my ($clusters) 	= $r->get(q{as.numeric(k)});
	my ($names)		= $r->get(q{rownames(M)});
	my ($col)		= $r->get(q{col});
    ...
    for (my $j=0; $j<@$clusters; $j++) {
        ...
        print K join("\t", $names->[$j], "cluster_".$clusters->[$j], $col->[$j]),"\n";
        push @{$CLUSTERS{$clusters->[$j]}},$names->[$j];
    }
	$r->send(qq{pdf("$opts_o/$opts_n.hclust.$criterion.pdf",width=12,height=9)});
    $r->send(q{layout(matrix(c(1,2),nrow=2), height=c(3,1))});
	$r->send(qq{plot(h,main="Hierarchical clustering of $opts_n with $criterion",labels=F)});
	$r->send(qq{rect.hclust(h,h=$opts_d,border="red")}) if $opts_d;
    $r->send(q{plotColorUnderTree(h,colors=data.frame(modules=col))});
	$r->send(q{dev.off()});
	print "Clustered ", scalar @IDs, " sequences with $criterion clustering into ", $r->get(q{length(unique(k))}), " clusters\n";
	$r->send(q{rm(m,k,h)});
}
...
print "Drew motif/domain profiles. Done\n" if $opts_v;
exit;
...
sub draw_profiles {
    ...
    foreach my $k (sort {$MAX{$b}{score}<=>$MAX{$a}{score} } @$ids) {
        ...
        print $OUT join("\t", "cluster_".$cluster, $k, $MAX{$k}{seq}, $MAX{$k}{score}, join(",",map {$_->display_name} @{$motifs{$k}}),join(",",map {$_->display_name} @{$motifs{$MAX{$k}{seq}}})),"\n" if $OUT ne "NO" && (!$SEEN{$k} || !$SEEN{b});
        $SEEN{$k}++;
        $SEEN{$b}++;
    }
